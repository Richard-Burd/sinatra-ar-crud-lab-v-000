CUSTOM  NOTES ON THIS PROJECT:
  Here are some important commands:
    1.) rake db:create_migration NAME=create_posts
    2.) rake db:migrate
  ...The default lab doesn't have the model "post" really doing anything
  because it is only doing stuff in active Record; it doesn't perform
  it's own functions like models in previous labs.
